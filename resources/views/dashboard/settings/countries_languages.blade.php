    ...
    <div class="row mb-6">
        ...
        <div class="col-lg-10 fv-row fv-plugins-icon-container">
            <select name="default_locale" data-control="select2" class="form-select form-select-solid form-select-lg fw-bold select2-hidden-accessible" data-select2-id="select2-data-10-jdo1-locale" tabindex="-1" aria-hidden="true">
                <option @if($submitFormMethod == 'put' && $settings->default_supported_locale == 'en') {{'selected'}} @endif value="en">{{trans(config('dashboard.trans_file').'english')}}</option>
                <option @if($submitFormMethod == 'put' && $settings->default_supported_locale == 'ar') {{'selected'}} @endif value="ar">{{trans(config('dashboard.trans_file').'arabic')}}</option>
            </select>
