...
<div class="row">
    ...
    <div class="card col-10 mb-5 mb-xl-10">
        ...
        <div id="kt_account_profile_details" class="collapse show">
            <!--begin::Form-->
            <form action="{{$submitFormRoute}}" method="POST" id="cityForm" class="form fv-plugins-bootstrap5 fv-plugins-framework" novalidate="novalidate">
                ...
                <div class="card-body border-top p-9">
                    ...
                    <div class="row mb-6 tab-pane fade show active" id="name_en" role="tabpanel" aria-labelledby="name_en-tab">
                        ...
                        <div class="col-lg-10 fv-row fv-plugins-icon-container">
                            <input type="text" name="name[en]" class="form-control form-control-lg form-control-solid" placeholder="{{trans(config('dashboard.trans_file').'name_en')}}" value="{{$submitFormMethod == 'put' ? $city->getTranslation('name', 'en') : old('name["en"]')}}">
                    ...
                    <div class="row mb-6 tab-pane fade d-none" id="name_ar" role="tabpanel" aria-labelledby="name_ar-tab">
                        ...
                        <!--begin::Col-->
                        <div class="col-lg-10 fv-row fv-plugins-icon-container">
                            <input type="text" name="name[ar]" class="form-control form-control-lg form-control-solid" placeholder="{{trans(config('dashboard.trans_file').'name_ar')}}" value="{{$submitFormMethod == 'put' ? $city->getTranslation('name', 'ar') : old('name["ar"]')}}">
                        ...
                        <div class="col-lg-10 fv-row fv-plugins-icon-container">
                            <select id="country_id" name="country_id" data-control="select2" class="form-select form-select-solid form-select-lg fw-bold select2-hidden-accessible" data-select2-id="select2-data-10-jdo1-country" tabindex="-1" aria-hidden="true">
                                @foreach($countries as $country)
                                    <option @if($submitFormMethod == 'put' && $city->country_id == $country->id) {{'selected'}} @endif value="{{$country->id}}">{{$country->name}}</option>
                                @endforeach
                            </select>
...
@push('footer-scripts')
    <script>
        ...
        $('#saveBtn').on('click', function(){
            ...
            submitForm($('#cityForm')[0], beforeSend, afterComplete, successResponse, errorResponse)
        })
        // === End script ===
    </script>
@endpush
