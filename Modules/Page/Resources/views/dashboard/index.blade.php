...
<ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
    ...
    <!--end::Item-->
    <!--begin::Item-->
    <li class="breadcrumb-item text-dark">{{trans('page::dashboard.pages')}}</li>
    <!--end::Item-->
</ul>
<!--end::Breadcrumb-->
@endsection
<div class="card card-custom">
    <div class="card-header">
        <div class="card-title">
            <h3 class="card-label">{{trans('page::dashboard.pages')}}</h3>
            <span class="text-muted mt-1 fw-bold fs-7">{{trans('page::dashboard.total_results', ['val' => $totalResults])}}</span>
        </div>
            ...
            <form class="mb-15" action="{{route('pages.index')}}" method="GET">
                <div class="row mb-6">
                    <div class="col-lg-3 mb-lg-0 mb-6">
                        <label>{{trans('page::dashboard.search_keyword')}}</label>
                        <input type="text" value="{{request()->search_keyword}}" name="search_keyword" class="form-control datatable-input" placeholder="{{trans('page::dashboard.search_keyword')}}" data-col-index="0">
                    </div>

                    <div class="col-lg-3 mb-lg-0 mb-6">
                        <label>{{trans('page::dashboard.status')}}</label>
                        <select name="status" class="form-select">
                            <option value="">{{trans('page::dashboard.all')}}</option>
                            <option @if(request()->status === '1') {{'selected'}} @endif value="1">{{trans('page::dashboard.active')}}</option>
                            <option @if(request()->status === '0') {{'selected'}} @endif value="0">{{trans('page::dashboard.deactivate')}}</option>
                        </select>
                    </div>

                    <div class="col-lg-3 mb-lg-0 mb-6">
                        <label>{{trans('page::dashboard.is_web')}}</label>
                        <select name="is_web" class="form-select">
                            <option value="">{{trans('page::dashboard.all')}}</option>
                            <option @if(request()->is_web === '1') {{'selected'}} @endif value="1">{{trans('page::dashboard.active')}}</option>
                            <option @if(request()->is_web === '0') {{'selected'}} @endif value="0">{{trans('page::dashboard.deactivate')}}</option>
                        </select>
                    </div>

                    <div class="col-lg-3 mb-lg-0 mb-6">
                        <label>{{trans('page::dashboard.is_mobile')}}</label>
                        <select name="is_mobile" class="form-select">
                            <option value="">{{trans('page::dashboard.all')}}</option>
                            <option @if(request()->is_mobile === '1') {{'selected'}} @endif value="1">{{trans('page::dashboard.active')}}</option>
                            <option @if(request()->is_mobile === '0') {{'selected'}} @endif value="0">{{trans('page::dashboard.deactivate')}}</option>
                        </select>
                    </div>
                </div>
                <div class="row mt-8">
                ...
                <table id="kt_datatable_example_1" class="table table-striped table-bordered table-hover table-checkable dataTable dtr-inline">
                    ...
                    <thead>
                        <tr class="fw-bold fs-6 text-muted">
                            ...
                            <th class="min-w-150px" data-column-name="title">{{trans('page::dashboard.title')}}</th>
                            <th class="min-w-100px" data-column-name="is_web">{{trans('page::dashboard.is_web')}}</th>
                            <th class="min-w-100px" data-column-name="is_mobile">{{trans('page::dashboard.is_mobile')}}</th>
                            <th class="min-w-100px" data-column-name="status">{{trans('page::dashboard.status')}}</th>
                            @if(auth()->guard('admin')->user()->can('page-edit') || auth()->guard('admin')->user()->can('page-delete'))
                            <th class="min-w-190px" data-column-name="operation">{{trans('page::dashboard.actions')}}</th>
                            @endif
                        </tr>
                    </thead>
                    <!--end::Table head-->
...
<script>
    var title = "{{trans('page::dashboard.delete_question')}}";
    ...
    function loadData()
    {
        var dataTable = $("#kt_datatable_example_1").DataTable({
            ...
            buttons: [
                {extend: 'collection', text: "{{trans('page::dashboard.export')}}", className:"export_btn btn btn-light", fade: true,
                    buttons:[
                        {extend: "pageLength", className: 'btn btn-default btn-sm-menu', text: "{{trans('page::dashboard.page_length')}}", exportOptions: {stripHtml: true, columns: ':visible'}},
                        {extend: "print", className: 'btn btn-default btn-sm-menu', text: "{{trans('page::dashboard.print')}}", exportOptions: {stripHtml: true, columns: ':visible'}},
                        {
                            text: "{{trans('page::dashboard.pdf')}}",
                        ...
                        {extend: "excel", className: 'btn btn-default btn-sm-menu', text: "{{trans('page::dashboard.excel')}}", exportOptions: {stripHtml: true, columns: ':visible'}},
                        {extend: "colvis", className: 'btn btn-default btn-sm-menu', text: "{{trans('page::dashboard.columns')}}", exportOptions: {stripHtml: true, columns: ':visible'}},
                    ]
                }
            ],
        });
