            ...
            columnDefs: [
                ...
                {targets: -1, orderable: false, searchable: false, render: function (data, type, full, meta) {
                    ...
                    @if(auth()->guard('admin')->user()->can('role-edit'))
                        var editRoute = '{{ route("roles.edit", ":id") }}'.replace(':id', data);
                        editUrl = '<a href="'+editRoute+'" class="btn btn-sm btn-clean btn-icon"><i class="edit_row la la-edit"></i></a>';
                    @endif
                    @if(auth()->guard('admin')->user()->can('role-delete'))
                        var deleteRoute = '{{ route("roles.destroy", ":id") }}'.replace(':id', data);
                    ...
                    buttons:[
                        ...
                        {extend: "print", className: 'btn btn-default btn-sm-menu', text: "{{trans('role::dashboard.print')}}", exportOptions: {stripHtml: true, columns: ':visible'}},
                        {
                            ...
                            action: function()
                            {
                                ...
                                $.ajax({
                                    type: 'GET',
                                    url: "{{route('download-role-pdf')}}?"+$.param($('#kt_datatable_example_1').DataTable().ajax.params()),
                                    contentType: "application/json",
                                    dataType: "json",
