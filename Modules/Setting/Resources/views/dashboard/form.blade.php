...
@section('page_path')
<!--begin::Title-->
<h1 class="d-flex align-items-center text-dark fw-bolder my-1 fs-3">{{trans('setting::dashboard.settings')}}</h1>
<!--end::Title-->
<!--begin::Separator-->
...
<ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
    ...
    <li class="breadcrumb-item text-muted">
        <a href="{{route('admin-home')}}" class="text-muted text-hover-primary">{{trans('setting::dashboard.home')}}</a>
    </li>
    <!--end::Item-->
    <!--begin::Item-->
...
<div class="row">
    ...
    @include('setting::dashboard.side_menu')
            ...
            <form action="{{$submitFormRoute}}" method="POST" id="countryForm" class="form fv-plugins-bootstrap5 fv-plugins-framework" novalidate="novalidate">
                ...
                <div class="card-body border-top p-9">
                    <div class="tab-content">
                        @include('setting::dashboard.logo_tab')
                        @include('setting::dashboard.appearance_tab')
                        @include('setting::dashboard.mail_configuration')
                        @include('setting::dashboard.contacts_info')
                        @include('setting::dashboard.social_media')
                        @include('setting::dashboard.custome_css_js')
                        @include('setting::dashboard.countries_languages')
                    </div>
                </div>
                <!--end::Card body-->
                <!--begin::Actions-->
                <div class="card-footer d-flex justify-content-end py-6 px-9">
                    <button type="reset" class="btn btn-white btn-active-light-primary me-2" onclick="window.location.reload()">{{trans('setting::dashboard.cancel')}}</button>
                    <button type="submit" class="btn btn-primary" id="saveBtn">
                        <span class="spinner-border spinner-border-sm align-middle ms-2 d-none"></span>
                        {{trans('setting::dashboard.save')}}
                    </button>
                </div>
                <!--end::Actions-->
            </form>
