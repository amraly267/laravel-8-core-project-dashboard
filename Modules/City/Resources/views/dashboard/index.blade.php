...
<ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
    ...
    <!--end::Item-->
    <!--begin::Item-->
    <li class="breadcrumb-item text-dark">{{trans('city::dashboard.cities')}}</li>
    <!--end::Item-->
</ul>
<!--end::Breadcrumb-->
@endsection
<div class="card card-custom">
    <div class="card-header">
        <div class="card-title">
            <h3 class="card-label">{{trans('city::dashboard.cities')}}</h3>
            <span class="text-muted mt-1 fw-bold fs-7">{{trans('city::dashboard.total_results', ['val' => $totalResults])}}</span>
        </div>
...
<div class="col-xl-12">
    ...
    <div class="card card-xxl-stretch mb-5 mb-xl-8">
        ...
        <div class="card-body py-3">
            <form class="mb-15" action="{{route('cities.index')}}" method="GET">
                <div class="row mb-6">
                    <div class="col-lg-4 mb-lg-0 mb-6">
                        <label>{{trans('city::dashboard.search_keyword')}}</label>
                        <input type="text" value="{{request()->search_keyword}}" name="search_keyword" class="form-control datatable-input" placeholder="{{trans('city::dashboard.search_keyword')}}" data-col-index="0">
                    </div>
                    <div class="col-lg-4 mb-lg-0 mb-6">
                        <label>{{trans('city::dashboard.country')}}</label>
                        <select name="country_id" class="form-select">
                            <option value="">{{trans('city::dashboard.all')}}</option>
                    ...
                    <div class="col-lg-4 mb-lg-0 mb-6">
                        <label>{{trans('city::dashboard.status')}}</label>
                        <select name="status" class="form-select">
                            <option value="">{{trans('city::dashboard.all')}}</option>
                            <option @if(request()->status === '1') {{'selected'}} @endif value="1">{{trans('city::dashboard.active')}}</option>
                            <option @if(request()->status === '0') {{'selected'}} @endif value="0">{{trans('city::dashboard.deactivate')}}</option>
                        </select>
                    </div>
                </div>
                <div class="row mt-8">
                ...
                <table id="kt_datatable_example_1" class="table table-striped table-bordered table-hover table-checkable dataTable dtr-inline">
                    ...
                    <thead>
                        <tr class="fw-bold fs-6 text-muted">
                            ...
                            <th class="min-w-100px" data-column-name="name">{{trans('city::dashboard.name')}}</th>
                            <th class="min-w-100px" data-column-name="country">{{trans('city::dashboard.country')}}</th>
                            <th class="min-w-100px" data-column-name="status">{{trans('city::dashboard.status')}}</th>
                            @if(auth()->guard('admin')->user()->can('city-edit') || auth()->guard('admin')->user()->can('city-delete'))
                            <th class="min-w-100px" data-column-name="operation">{{trans('city::dashboard.actions')}}</th>
                            @endif
                        </tr>
                    </thead>
                    <!--end::Table head-->
...
<script>
    var title = "{{trans('city::dashboard.delete_question')}}";
    ...
    function loadData()
    {
        var dataTable = $("#kt_datatable_example_1").DataTable({
            ...
            columns:[{data: 'index'}, {data: 'name'}, {data: 'country'}, {data: 'status'}, {data: 'action'}],
            dom: 'frtipB',
            ...
            columnDefs: [
                ...
                {targets: -1, orderable: false, searchable: false, render: function (data, type, full, meta) {
                    ...
                    @if(auth()->guard('admin')->user()->can('city-edit'))
                        var editRoute = '{{ route("cities.edit", ":id") }}'.replace(':id', data);
                    ...
                    @if(auth()->guard('admin')->user()->can('city-delete'))
                        var deleteRoute = '{{ route("cities.destroy", ":id") }}'.replace(':id', data);
            ...
            buttons: [
                {extend: 'collection', text: "{{trans('city::dashboard.export')}}", className:"export_btn btn btn-light", fade: true,
                    buttons:[
                        {extend: "pageLength", className: 'btn btn-default btn-sm-menu', text: "{{trans('city::dashboard.page_length')}}", exportOptions: {stripHtml: true, columns: ':visible'}},
                        {extend: "print", className: 'btn btn-default btn-sm-menu', text: "{{trans('city::dashboard.print')}}", exportOptions: {stripHtml: true, columns: ':visible'}},
                        {
                            text: "{{trans('city::dashboard.pdf')}}",
                            ...
                            action: function()
                            {
                                ...
                                $.ajax({
                                    type: 'GET',
                                    url: "{{route('download-city-pdf')}}?"+$.param($('#kt_datatable_example_1').DataTable().ajax.params()),
                                    contentType: "application/json",
                                    dataType: "json",
                        ...
                        {extend: "excel", className: 'btn btn-default btn-sm-menu', text: "{{trans('city::dashboard.excel')}}", exportOptions: {stripHtml: true, columns: ':visible'}},
                        {extend: "colvis", className: 'btn btn-default btn-sm-menu', text: "{{trans('city::dashboard.columns')}}", exportOptions: {stripHtml: true, columns: ':visible'}},
                    ]
                }
            ],
        });
